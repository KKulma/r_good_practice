...
knitr::include_graphics("pics/jb_tweet.png")
```
...
Your code will grow. Sometimes it will turn into an R script equivalent of the Bible, at least in terms of volume. To keep it organised, keep your code nice and tidy by assigning relevant code chunks to different sections that can be later folded/udfolded; you can also easily navigate yourself through chunks by using 'drop-up' menu at the bottom of the script screen. 

You create a new code section by writing `####` or `----` at the end of any comment that is to become a new code section.

The clip below will give you an idea what I'm talking about.  
...
I know, I know. We all mean to do it, but we rarely do. I always promise myself that I'll be thoroughtly commenting my code from the very beginning of the project but even these days I find it difficult to do it consistently. Why? Because:
...
knitr::include_graphics("pics/jenny1.png")
```
...
```{r, out.width = "600px", echo=FALSE}
knitr::include_graphics("pics/jenny2.png")
```
...
```{r, out.width = "600px", echo=FALSE}
knitr::include_graphics("pics/jenny3.png")
```
...
- left pad other numbers with zeros - otherwise you'll end up with `10` before `1` and so on.

Again, one of Jenny's examples:

```{r, out.width = "550px", echo=FALSE}
knitr::include_graphics("pics/jenny4.png")
```
...
To learn how to write good functions, you can have a sneak peak into the first chapter of the DataCamp course for free (the rest is avaialble under paid subscription) or read this [DataCamp tutorial](https://www.datacamp.com/community/tutorials/functions-in-r-a-tutorial). 



### **09. With big, complex data projects use project pipeline.**

I'm not sure if `project pipeline` is an official name for what I want to tak about, but for a sake of the argument, let's call it a `project pipeline` ;) Namely, sometimes running a full project from one script - with even most clear and informatively named code sections - is simply difficult, if not unfeasible. Particularly when big(ish) data is involved: imagine trying to run a hevty model on data that requires importing and cleaning, where import into R alone takes about an hour (real life scenario!). You wouldn't possibly want to do it every time you open the project, right? 
...
This almost goes without saying, but if I got a £ for every time I forgot to do it...
